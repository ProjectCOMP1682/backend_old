...
let sendSimpleEmail = async (dataSend) => {
    ...
    if (dataSend.type === 'verifyEmail') {
        let info = await transporter.sendMail({
            from: '"Greenwich Store" <sondvtgcd201619@fpt.edu.vn>', // sender address
            to: dataSend.email, // list of receivers
            subject: "Xác thực email | Greenwich Store", // Subject line
    ...
    if (dataSend.type === 'forgotpassword') {
        let info = await transporter.sendMail({
            from: '"Greenwich Store" <sondvtgcd201619@fpt.edu.vn>', // sender address
            to: dataSend.email, // list of receivers
            subject: "Xác nhận quên mật khẩu | Greenwich Store", // Subject line
